...
class KnowledgeBase:
    ...
    async def add_document(self, filename: str, content: str, user_id: UUID = None) -> str:
        """
        Ingest a document: track file -> chunk -> embed -> save.
        Returns the file ID.
        """
        ...
        async with get_async_session() as session:
            ...
            res = await session.execute(
                text("""
                    INSERT INTO knowledge_files (filename, file_size, content_hash, uploaded_by, status)
                    VALUES (:name, :size, :hash, :uid, 'processing')
                    RETURNING id
                """),
                {
                    "name": filename,
            ...
            try:
                ...
                # We do this sequentially or bounded parallel to respect rate limits
